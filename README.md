...
### Tool surface (92 tools across 16 groups)
...
| `cloud` (4) | `aws_cw_list_metrics`, `aws_cw_get_metric_statistics` *(CloudWatch)*, `azure_monitor_metric_definitions`, `azure_monitor_metrics` *(Azure Monitor)* — read-only via the operator's `aws`/`az` CLIs; no stored secrets. Registers when a `cloud_aws:` / `cloud_azure:` block is configured. GCP metric read is deferred (see `docs/RFC-CLOUD-OBSERVABILITY.md`) |
