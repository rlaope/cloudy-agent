// Package py provides read-only Python diagnostic tools for the cloudy SRE agent.
...
func (e *ExitError) Error() string {
	return fmt.Sprintf("py: %s exited %d: %s", e.Cmd, e.Code, e.Stderr)
}
...
func run(ctx context.Context, name string, args ...string) (stdout, stderr string, err error) {
	...
	if runErr != nil {
		...
		return stdout, stderr, fmt.Errorf("py: exec %s: %w", name, runErr)
	}
